...
/* ══════════════════════════════════════════════════════════
   TrackSpace Landing — Light Mode (same layout as dark ver)
   ══════════════════════════════════════════════════════════ */
...
const GLOW_BLUE = 'rgba(59,130,246,0.07)';
const GLOW_PURPLE = 'rgba(139,92,246,0.06)';

const LandingPage: React.FC = () => {
  return (
    <Box sx={{
      bgcolor: BG, color: TEXT, minHeight: '100vh',
      fontFamily: "'Inter', 'Segoe UI', sans-serif",
    }}>
      {/* ─── Navbar ─── */}
      <Box component="nav" sx={{
        position: 'fixed', top: 0, left: 0, right: 0, zIndex: 100,
        bgcolor: 'rgba(248,250,252,0.85)', backdropFilter: 'blur(20px)',
        ...
        <Container maxWidth="md" sx={{ textAlign: 'center', position: 'relative' }}>
          ...
          <Box sx={{ display: 'flex', gap: 2, justifyContent: 'center', flexWrap: 'wrap' }}>
            ...
            <Button href="#features" variant="outlined" size="large"
              sx={{
                px: 4, py: 1.5, fontWeight: 600, borderRadius: '12px',
                textTransform: 'none', fontSize: '1rem',
        ...
        <Container maxWidth="lg" sx={{ position: 'relative' }}>
          <Box sx={{ textAlign: 'center', mb: 7 }}>
            ...
            <Typography variant="h4" fontWeight={800} sx={{
              mb: 1.5, letterSpacing: '-0.02em', color: TEXT,
            }}>
            ...
            {FEATURES.map(f => (
              <Grid item xs={12} sm={6} md={4} key={f.title}>
                <Box sx={{
                  ...
                  <Typography variant="subtitle1" fontWeight={700} sx={{ mb: 0.75, color: TEXT }}>
                    {f.title}
                  </Typography>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            {STEPS.map((s, i) => (
              <Box key={s.num} sx={{
                ...
                <Box>
                  <Typography variant="subtitle1" fontWeight={700} sx={{ color: TEXT, mb: 0.5 }}>
                    {s.title}
                  </Typography>
                  <Typography variant="body2" sx={{ color: TEXT_DIM, lineHeight: 1.7 }}>
                    {s.desc}
      ...
      <Box sx={{ py: 12, textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        ...
        <Container maxWidth="sm" sx={{ position: 'relative' }}>
          <Box sx={{
            ...
            <Button component={Link} to="/login" variant="contained" size="large"
              sx={{
                px: 5, py: 1.5, fontWeight: 700, borderRadius: '12px',
      ...
      {/* ─── Footer ─── */}
      <Box sx={{ py: 3, borderTop: `1px solid ${BORDER}`, bgcolor: BG_CARD }}>
