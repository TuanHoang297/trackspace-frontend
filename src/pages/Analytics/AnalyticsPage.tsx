...
const MemberCard: React.FC<{
    ...
    const rankColor = rank < 3 ? RANK_COLORS[rank] : '#CBD5E1';
...
// ─── Scoring model overview panel ─────────────────────────────────────────────

// Row helper used in bonus/penalty tables
const RuleRow: React.FC<{ label: string; value: string; note: string; valueColor: string }> = ({ label, value, note, valueColor }) => (
    <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 1, py: 0.75, borderBottom: '1px solid #F1F5F9', '&:last-child': { borderBottom: 'none' } }}>
        <Box sx={{ flex: 1 }}>
            <Typography variant="body2" fontWeight={600}>{label}</Typography>
            <Typography variant="caption" color="text.disabled" sx={{ lineHeight: 1.4 }}>{note}</Typography>
        </Box>
        <Box sx={{ bgcolor: `${valueColor}18`, borderRadius: 1.5, px: 1, py: 0.25, flexShrink: 0, alignSelf: 'center' }}>
            <Typography variant="caption" fontWeight={800} sx={{ color: valueColor }}>{value}</Typography>
        </Box>
    </Box>
);

const ModelOverviewPanel: React.FC<{ feWeight: number }> = ({ feWeight }) => {
    const [open, setOpen] = useState(false);
    const beWeight = 100 - feWeight;

    return (
        <Box sx={{ bgcolor: '#FAFBFF', borderRadius: 3, border: '1px solid #E2E8F0', mb: 3, overflow: 'hidden' }}>

            {/* ── Summary row (always visible) ─────────────────────────────── */}
            <Box
                onClick={() => setOpen(o => !o)}
                sx={{ p: 2, display: 'flex', alignItems: 'center', gap: 2, cursor: 'pointer', flexWrap: 'wrap',
                      '&:hover': { bgcolor: '#F8FAFC' }, transition: 'background 0.15s' }}
            >
                <SpeedIcon sx={{ fontSize: 18, color: '#8B5CF6', flexShrink: 0 }} />
                <Box sx={{ flex: 1 }}>
                    <Typography fontWeight={700} variant="body2">Cách hệ thống chấm điểm đóng góp</Typography>
                    <Typography variant="caption" color="text.secondary">
                        Nhấn để xem giải thích chi tiết từng bước
                    </Typography>
                </Box>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75, flexWrap: 'wrap' }}>
                    <Chip size="small" icon={<CodeIcon sx={{ fontSize: 12 }} />} label={`Frontend ${feWeight}%`}
                        sx={{ bgcolor: '#EFF6FF', color: '#3B82F6', fontWeight: 700, fontSize: '0.7rem', height: 22,
                              '& .MuiChip-icon': { color: '#3B82F6' } }} />
                    <Typography variant="caption" color="text.disabled">+</Typography>
                    <Chip size="small" icon={<StorageIcon sx={{ fontSize: 12 }} />} label={`Backend ${beWeight}%`}
                        sx={{ bgcolor: '#F5F3FF', color: '#8B5CF6', fontWeight: 700, fontSize: '0.7rem', height: 22,
            ...
            {/* ── Expandable detail ────────────────────────────────────────── */}
            <Collapse in={open}>
                <Box sx={{ borderTop: '1px solid #E2E8F0', p: { xs: 2, sm: 2.5 }, display: 'flex', flexDirection: 'column', gap: 2.5 }}>
                    ...
                    <Box>
                        ...
                        <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                            {/* Step 1 */}
                            <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start', p: 1.5,
                                       bgcolor: '#EFF6FF', borderRadius: 2, border: '1px solid #BFDBFE' }}>
                                <Box sx={{ width: 28, height: 28, borderRadius: '50%', bgcolor: '#3B82F6', color: '#fff',
                                           fontWeight: 800, fontSize: '0.8rem', display: 'flex', alignItems: 'center',
                                ...
                                <Box>
                                    ...
                                    <Box sx={{ mt: 0.75, display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                                        <Chip size="small" icon={<CodeIcon sx={{ fontSize: 11 }} />}
                                            label="FE thi với FE"
                                            sx={{ bgcolor: '#DBEAFE', color: '#1D4ED8', fontWeight: 600, fontSize: '0.68rem', height: 20,
                                                  '& .MuiChip-icon': { color: '#1D4ED8' } }} />
                                        <Chip size="small" icon={<StorageIcon sx={{ fontSize: 11 }} />}
                                            label="BE thi với BE"
                                            sx={{ bgcolor: '#EDE9FE', color: '#6D28D9', fontWeight: 600, fontSize: '0.68rem', height: 20,
                                                  '& .MuiChip-icon': { color: '#6D28D9' } }} />
                                        <Typography variant="caption" color="text.disabled" sx={{ alignSelf: 'center' }}>
                                            → Hoàn toàn công bằng cho từng vị trí
                                        </Typography>
                                    </Box>
                                </Box>
                            </Box>
                            ...
                            {/* Step 2 */}
                            <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start', p: 1.5,
                                       bgcolor: '#FFFBEB', borderRadius: 2, border: '1px solid #FDE68A' }}>
                                <Box sx={{ width: 28, height: 28, borderRadius: '50%', bgcolor: '#F59E0B', color: '#fff',
                                           fontWeight: 800, fontSize: '0.8rem', display: 'flex', alignItems: 'center',
                                           justifyContent: 'center', flexShrink: 0, mt: 0.1 }}>2</Box>
                                <Box sx={{ flex: 1 }}>
                                    <Typography variant="body2" fontWeight={700} sx={{ color: '#B45309', mb: 0.3 }}>
                                        Chấm điểm nỗ lực code từ GitHub
                                    </Typography>
                                    <Typography variant="body2" color="text.secondary" sx={{ lineHeight: 1.6, mb: 1 }}>
                                        Hệ thống đọc lịch sử commit của từng sinh viên và tính điểm dựa trên{' '}
                                        <strong>số dòng code tự viết</strong>, <strong>loại file</strong>,
                                        <strong> có sửa lỗi không</strong>, và <strong>commit đều đặn hay dồn hết cuối kỳ</strong>.
                                        Dù copy 10.000 dòng từ thư viện cũng chỉ được điểm tương đương tự viết khoảng 40 dòng —
                                        hệ thống phát hiện ngay.
                                    </Typography>
                                    <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap' }}>
                                        {[
                                            { label: 'Commit bình thường', val: 'điểm gốc', c: '#94A3B8' },
                                            { label: 'Có từ "fix / bug / resolve"', val: '× 2 điểm', c: '#F59E0B' },
                                            { label: 'Có từ "hotfix"', val: '× 3 điểm', c: '#EF4444' },
                                            { label: 'Commit đều ≥ 14 ngày', val: '× 2 điểm thưởng', c: '#10B981' },
                                            { label: 'Dồn commit cuối kỳ', val: '× 0.5 điểm phạt', c: '#6B7280' },
                                        ].map(r => (
                                            <Box key={r.label} sx={{ display: 'flex', alignItems: 'center', gap: 0.5,
                                                                      bgcolor: `${r.c}12`, borderRadius: 1.5, px: 1, py: 0.4 }}>
                                                <Typography variant="caption" color="text.secondary">{r.label}</Typography>
                                                <Typography variant="caption" fontWeight={800} sx={{ color: r.c }}>→ {r.val}</Typography>
                                            </Box>
                                        ))}
                                    </Box>
                                </Box>
                            ...
                            {/* Step 3 */}
                            <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start', p: 1.5,
                                       bgcolor: '#F5F3FF', borderRadius: 2, border: '1px solid #DDD6FE' }}>
                                <Box sx={{ width: 28, height: 28, borderRadius: '50%', bgcolor: '#8B5CF6', color: '#fff',
                                           fontWeight: 800, fontSize: '0.8rem', display: 'flex', alignItems: 'center',
                                           justifyContent: 'center', flexShrink: 0, mt: 0.1 }}>3</Box>
                                <Box sx={{ flex: 1 }}>
                                    <Typography variant="body2" fontWeight={700} sx={{ color: '#6D28D9', mb: 0.3 }}>
                                        Chấm điểm tiến độ thực tế từ Jira
                                    </Typography>
                                    <Typography variant="body2" color="text.secondary" sx={{ lineHeight: 1.6, mb: 1 }}>
                                        Nền điểm là <strong>tỉ lệ task đã hoàn thành (Done) / tổng task được giao</strong>.
                                        Nếu sinh viên báo xong nhưng khi test ra lỗi và phải làm lại,
                                        điểm chất lượng sẽ bị trừ. Ngược lại, ai viết code ngắn gọn mà đóng được nhiều task
                                        sẽ được thưởng thêm.
                                    </Typography>
                                    <Box sx={{ display: 'flex', gap: 1.5, flexWrap: 'wrap' }}>
                                        {[
                                            { label: 'Không bị làm lại lần nào', val: 'chất lượng 100%', c: '#10B981' },
                                            { label: 'Bị làm lại 1 lần', val: 'chất lượng −15%', c: '#F59E0B' },
                                            { label: 'Bị làm lại 2 lần', val: 'chất lượng −30%', c: '#EF4444' },
                                            { label: 'Bị làm lại ≥ 3 lần', val: 'chất lượng −50%', c: '#DC2626' },
                                            { label: 'Code ngắn, task nhiều', val: 'thưởng tối đa +50%', c: '#3B82F6' },
                                        ].map(r => (
                                            <Box key={r.label} sx={{ display: 'flex', alignItems: 'center', gap: 0.5,
                                                                      bgcolor: `${r.c}12`, borderRadius: 1.5, px: 1, py: 0.4 }}>
                                                <Typography variant="caption" color="text.secondary">{r.label}</Typography>
                            ...
                            {/* Step 4 */}
                            <Box sx={{ display: 'flex', gap: 1.5, alignItems: 'flex-start', p: 1.5,
                                       bgcolor: '#ECFDF5', borderRadius: 2, border: '1px solid #A7F3D0' }}>
                                <Box sx={{ width: 28, height: 28, borderRadius: '50%', bgcolor: '#10B981', color: '#fff',
                                           fontWeight: 800, fontSize: '0.8rem', display: 'flex', alignItems: 'center',
                                           justifyContent: 'center', flexShrink: 0, mt: 0.1 }}>4</Box>
                                <Box sx={{ flex: 1 }}>
                                    <Typography variant="body2" fontWeight={700} sx={{ color: '#065F46', mb: 0.3 }}>
                                        Tổng hợp thành 1 điểm duy nhất (0 – 100)
                                    </Typography>
                                    <Typography variant="body2" color="text.secondary" sx={{ lineHeight: 1.6, mb: 1.25 }}>
                                        Điểm cuối = trung bình cộng của điểm GitHub và điểm Jira.
                                        Giảng viên có thể điều chỉnh thanh trượt <strong>FE% / BE%</strong> trên đây
                                        để đặt trọng số phù hợp với đặc thù dự án, rồi nhấn <strong>"Tính lại"</strong>.
                                    </Typography>
                                    {/* Visual equation */}
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, flexWrap: 'wrap' }}>
                                        <Box sx={{ px: 2, py: 1, bgcolor: '#EFF6FF', borderRadius: 2, border: '1.5px solid #93C5FD', textAlign: 'center' }}>
                                            <Typography variant="caption" color="text.disabled" sx={{ display: 'block' }}>Điểm GitHub</Typography>
                                            <Typography fontWeight={800} sx={{ color: '#3B82F6', fontSize: '1rem' }}>50%</Typography>
                                        </Box>
                                        <Typography fontWeight={800} color="text.disabled" sx={{ fontSize: '1.1rem' }}>+</Typography>
                                        <Box sx={{ px: 2, py: 1, bgcolor: '#F5F3FF', borderRadius: 2, border: '1.5px solid #C4B5FD', textAlign: 'center' }}>
                                            <Typography variant="caption" color="text.disabled" sx={{ display: 'block' }}>Điểm Jira</Typography>
                                            <Typography fontWeight={800} sx={{ color: '#8B5CF6', fontSize: '1rem' }}>50%</Typography>
                                        </Box>
                                        <Typography fontWeight={800} color="text.disabled" sx={{ fontSize: '1.1rem' }}>=</Typography>
                                        <Box sx={{ px: 2, py: 1, bgcolor: '#ECFDF5', borderRadius: 2, border: '1.5px solid #6EE7B7', textAlign: 'center' }}>
                                            <Typography variant="caption" color="text.disabled" sx={{ display: 'block' }}>Điểm Đóng Góp</Typography>
                                            <Typography fontWeight={800} sx={{ color: '#10B981', fontSize: '1rem' }}>0 – 100</Typography>
                                        </Box>
                                    </Box>
                                </Box>
                            </Box>
                    ...
                    {/* ── Quick reference: what affects score up/down ── */}
                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 2 }}>
                        {/* Được thêm điểm */}
                        <Box sx={{ bgcolor: '#F0FDF4', borderRadius: 2, border: '1px solid #BBF7D0', p: 1.5 }}>
                            <Typography variant="caption" fontWeight={800} sx={{ color: '#15803D', display: 'block', mb: 1 }}>
                                ✅ ĐƯỢC THÊM ĐIỂM KHI...
                            </Typography>
                            {[
                                { label: 'Commit sửa lỗi (fix / bug / hotfix)', note: 'Nhân 2× hoặc 3× điểm commit đó' },
                                { label: 'Commit mỗi ngày liên tục ≥ 14 ngày', note: 'Nhân 2× toàn bộ điểm GitHub' },
                                { label: 'Hoàn thành nhiều task, code ngắn gọn', note: 'Thưởng tối đa +50% điểm Jira' },
                                { label: 'Không bị làm lại task nào', note: 'Điểm chất lượng Jira giữ 100%' },
                            ].map(r => (
                                <RuleRow key={r.label} label={r.label} note={r.note} value="+" valueColor="#16A34A" />
                            ))}
                        </Box>
                        {/* Bị trừ điểm */}
                        <Box sx={{ bgcolor: '#FFF7ED', borderRadius: 2, border: '1px solid #FED7AA', p: 1.5 }}>
                            <Typography variant="caption" fontWeight={800} sx={{ color: '#C2410C', display: 'block', mb: 1 }}>
                                ⚠️ BỊ TRỪ ĐIỂM KHI...
                            </Typography>
                            {[
                                { label: 'Dồn commit vào cuối kỳ, ngày thường ít', note: 'Nhân 0.5× — phạt không đều đặn' },
                                { label: 'Task bị test lỗi, phải làm đi làm lại', note: '−15% mỗi lần, sàn −50% khi ≥ 3 lần' },
                                { label: 'Copy code từ thư viện số lượng lớn', note: 'Log₁₀: 10.000 dòng copy ≈ 40 dòng tự viết' },
                                { label: 'File config / tài liệu chiếm đa số', note: 'Trọng số 0.1× — ít được ghi nhận hơn' },
                            ].map(r => (
                                <RuleRow key={r.label} label={r.label} note={r.note} value="−" valueColor="#EA580C" />
                            ))}
                        </Box>
                    </Box>
